...
import { executeWithCredit } from '../services/creditService';
...
export const IdeaGenerator: React.FC<IdeaGeneratorProps> = ({ state, onStateChange, onDeductCredits, userCredits }) => {
  ...
  const handleGenerateStructure = async () => {
    ...
    try {
        const images = await executeWithCredit(
            'user-uuid-1234',
            'IDEA_GENERATOR',
            numberOfImages,
            async () => {
                return await geminiService.generateIdeaStructure(sketchImage, styleImage || null, numberOfImages, (status) => setProcessStatus(status));
            }
        );
        ...
        if (error.message === 'INSUFFICIENT_CREDITS') {
             alert('Số dư Credit không đủ để thực hiện. Vui lòng nạp thêm.');
        } else {
             alert(`Lỗi tạo khung sườn: ${error.message}`); 
        }
    } 
    ...
    try {
        const images = await executeWithCredit(
            ...
            async () => {
                ...
                return await geminiService.generateIdeaDecor(
                    composite.base64, 
                    composite.mimeType, 
                    numberOfImages, 
        ...
        if (error.message === 'INSUFFICIENT_CREDITS') {
             ...
             alert(`Lỗi ghép đồ: ${error.message}`); 
        }
    } 
    finally { setIsProcessing(false); }
  };
